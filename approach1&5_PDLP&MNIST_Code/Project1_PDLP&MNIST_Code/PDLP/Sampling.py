...
def make_gaussian_samples(N_dists=4,
                          n_points=60,
                          dim=2,
                          seed=0, uniform_weights=False):
    """
    ...
    for _ in range(N_dists):
        ...
        m_t = n_points
        # rng.integers(n_points_range[0], n_points_range[1] + 1)

        # Randomly generate a mean and std deviation per coordinate
        mean = rng.uniform(-10, 10, size=dim)
        std = rng.uniform(5, 8, size=dim)
...
Gaussian_distributions = make_gaussian_samples(seed=241)
A, b, c, meta = barycenter_lp_matrices(Gaussian_distributions)
save_sample_hdf5("sample_003.h5", Gaussian_distributions, A, b, c, meta)
