function [c, OT, iter_hist, optval_hist] = Wasserstein_Barycenter(db, c0, options)
...
%           instances by colums at phase i; 这
...
for s=1:n
    ...
    elseif strcmp(method, 'free_maaipm')
        tic;
        T0=toc;
        [c{s}, iter, optval,iter_hist, optval_hist]=centroid_sphFreeMAAIPM(db{s}.stride, db{s}.supp, db{s}.w, c0{s}, options);
        t=toc;
        t = t - T0;
        fprintf('Free support MAAIPM needs %5.2fs\n',t);
        fprintf(resultfile,'primal IPM, %d, %4.3f, %2.8f\n',iter, t, optval);
    elseif strcmp(method, 'fixed_maaipm')
        [c{s}, iter, optval,t,fea, iter_hist, optval_hist]=centroid_sphFixedMAAIPM(db{s}.stride, db{s}.supp, db{s}.w, c0{s}, options);
    ...
    figure;
plot(iter_hist, optval_hist, '-o', 'LineWidth', 2);
xlabel('Iteration');
ylabel('Objective Value');
title('IPM Convergence Curve');
grid on;
