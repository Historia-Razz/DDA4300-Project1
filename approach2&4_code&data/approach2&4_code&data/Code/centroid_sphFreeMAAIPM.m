function [IPMx, iter, optval, iter_hist, optval_hist] = centroid_sphFreeMAAIPM(stride, supp, w, c0, options)
%Free support case MAAIPM
% reference:
% Ge, D., Wang, H., Xiong, Z., & Ye, Y. (2019).
% Interior-point Methods Strike Back: Solving the Wasserstein Barycenter Problem.
% arXiv preprint arXiv:1905.12895.
% As for the framework of the IPM, see chapter 14 of
% Nocedal, J., & Wright, S. (2006). Numerical optimization. Springer Science & Business Media.
% This code has been created by Zikai Xiong (zikai.xiong [AT] outlook.com)
% and Haoyue Wang (wanghaoyue3 [AT] outlook.com)
tic;
...
while (bigloop<=begin_primal + time_primal_max)
    ...
    for iter=1:itmax
        ...
        dx = ( (A_true'*dp).*x+t1)./s;
        ds = -(s.*dx+Rc)./x;
        eta = max(etaMin,1-mu); % 设置最大步长分数到边界的参数eta
        ...
        dx = ( (A_true'*dp).*x+t1)./s;
        ds = -(s.*dx+Rc)./x;
        [alpha, alphax, alphas] = steplength(x, s, dx, ds, eta); %寻找步长alpha
        x = x + alphax * dx;
        s = s + alphas * ds;
        p = p + alphas * dp;
    ...
    for iter=1:100
        ...
        iter_hist(end+1)   = iter;
        optval_hist(end+1) = Poptval;
