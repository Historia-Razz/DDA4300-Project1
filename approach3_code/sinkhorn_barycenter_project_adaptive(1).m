...
function [X, a, objective_values] = free_support_barycenter(Y_list, b_list, a_init, X_init, stopThr, reg, beta, theta, t_0, numItermax, verbose)
    ...
    for k = 1:numItermax
        ...
        [a, X, obj] = solve_barycenter(X, Y_list, a, b_list, M_list, reg_k, beta, theta, t_0);
        ...
        if dX < 5e-3 && da < 1e-4 && objective_gap < 1e-4
            break;
        end
    end
end
...
N = 15;
file_name = 'test_N15_mt20.d2';
d_modalities = 2
db = loaddata(N, 1, d_modalities, file_name);
...
for i = 1:db{1}.N
    ...
    Y_list{i} = reshape(points, mi, d_modalities); % Reshape to (mi, 2) for x and y coordinates
...
% Initialize support
num_support = 20;
X_init = -5 + 10 * rand(num_support, d_modalities);
...
tic;
t0 = toc;
[X_final, a_final, objective_values] = free_support_barycenter(Y_list, b_list, a_init, X_init, 1e-3, 0.01, 0.5, 0.7, 1e-10, 500, true);
T = toc;
T = T - t0;
...
% X_init(2, 1) = 1+1e-2;
% a_init = zeros(2, 1);
% a_init(1, 1) = 0.01+1e-2;
% a_init(2, 1) = 0.99-1e-2;
% 
% % Solve barycenter
% tic;
% t0 = toc;
...
% T = T - t0;
% fprintf('time needed:%.2f\n', T);
% fprintf('final_X:{%.4f,%.4f}\n', X_final(1), X_final(2));
% fprintf('final_w:(%.4f, %.4f)\n', a_final(1), a_final(2));